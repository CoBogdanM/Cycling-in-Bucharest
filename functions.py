...
plotparam = {"bbox": (1280,1280),
			"dpi": 96,
            "SUMP_existing_routes": {"width": 1, "edge_color": 'Blue'},
            "SUMP_riders_preferences_routes": {"width": 1, "edge_color": 'Green'},
            "SUMP_transport_hubs_routes": {"width": 1, "edge_color": 'Red'},
            "SUMP_employment_hubs_routes": {"width": 1, "edge_color": '#6d4d41'},
            "SUMP_commercial_hubs_routes": {"width": 1, "edge_color": 'Purple'},
            "SUMP_connectivity_routes": {"width": 1, "edge_color": 'Orange'},
...
def nx_to_ig(nx_graph:nx):
    ...
    ig_graph.add_edges([(node_mapping[u], node_mapping[v]) for u, v, _ in edges])
    ig_graph.vs["id"] = [attr.get("osmid", node_id) for node_id, attr in nx_graph.nodes(data=True)]
    for edge in edges:
        u, v, attributes = edge
        edge_id = ig_graph.get_eid(node_mapping[u], node_mapping[v])
        for key, value in attributes.items():
            ig_graph.es[edge_id][key] = value
    ig_graph.es["weight"] = [data.get("length", 1) for _, _, data in nx_graph.edges(data=True)]
    return ig_graph
...
def ox_to_csv(G, p, placeid, parameterid, postfix = "", compress = True, verbose = True):
    ...
    if verbose: print(f"{placeid}: Successfully wrote graph{parameterid}{postfix}")
...
def calculate_metrics(G, GT_abstract, G_big, population_squares ,nnids, calcmetrics = {"length":0,
    ...
    if G.ecount() > 0 and GT_abstract.ecount() > 0:
        ...
        if "directness_bicycle_car" in calcmetrics:
            output["directness_bicycle_car"], output["directness_bicycle_car_reachability"] = calculate_bicycle_car_directness(G, G_big, numnodepairs)
        ...
        if "directness_all_linkwise" in calcmetrics:
            if "directness_lcc_linkwise" in calcmetrics and len(cl) <= 1:
                ...
                output["directness_all_linkwise"], output["directness_all_linkwise_reachability"] = calculate_directness_linkwise(G, numnodepairs) # number of components is checked within calculate_directness_linkwise()
        # Population Covered
